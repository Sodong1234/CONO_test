...
<script type="text/javascript">
	function dealCancel(item_idx, safe_status){
		var dealConfirm = confirm("거래 취소 하시겠습니까?" + '\n' + "거래를 되돌릴 수 없습니다.");
		
		if(dealConfirm){
			location.href="AdminDealCancel.admin?item_idx=" + item_idx + "&coin_use=" + coin_use + "&safe_status=" + safe_status;
		}
	}
		...
		<c:if test="${not empty dealList and pageInfo.getListCount() > 0 }">
			<c:forEach var="product" items="${dealList }">
				<tr>
					...
					<td>${product[5] }</td>
					<td>${product[6] }</td>
					<td><input type="button" value="거래취소" onclick="dealCancel('${product[1] }', '${product[4] }', '${product[6] }')"></td>
				</tr>
			</c:forEach>
		</c:if>
	</table>
	...
	<section>
		<c:choose>
			<c:when test="${pageNum > 1}">
				<input type="button" value="이전" onclick="location.href='AdminDealList?page=${pageNum - 1}'">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${pageNum < maxPage}">
				<input type="button" value="다음" onclick="location.href='AdminDealList?page=${pageNum + 1}'">
			</c:when>
			<c:otherwise>
