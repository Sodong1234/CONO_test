...
public class ItemModifyProAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("ItemModifyProAction");
...
//			System.out.println(filename + ", " + orgFileName);
		...
		item.setItem_idx(multi.getParameter("item_idx"));
...
//		item.setItem_region(multi.getParameter("item_region"));
		...
		boolean isModifySuccess = service.ModifyItem(item, imgList, member_id, categoryDTO);
		
		// 이전 이미지 실제 이름 
		String[] preImgNames = multi.getParameter("preImgName").split("/");
		
		// 수정된 이미지 실제 이름
		ArrayList<String> newImgList = service.selectImgList(item.getItem_idx());
		
		File file = null;
		for(String preImgName : preImgNames) {
			boolean isDelete = false;
			for(String newImg : newImgList) {
				if(preImgName.equals(newImg)) {
//					System.out.println(preImgName + ", " + newImg + " : true");
//					System.out.println(preImgName.equals(newImg));
					isDelete = false;
					break;
				} else {
//					System.out.println(preImgName + ", " + newImg + " : false");
//					System.out.println(preImgName.equals(newImg));
					isDelete = true;
				}
			}
			if(isDelete) {
				file = new File("D:\\workspace_Cono\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\Cono_lsy\\upload\\" + preImgName);
				if(file.exists()) {
					file.delete();
				}
			}
		...
		if(isModifySuccess) {
			...
			forward.setPath("ItemDetail.shop?item_idx=" + multi.getParameter("item_idx"));
