...
<head>
<meta charset="UTF-8">
<title>myshop/item_order_finish_mng.jsp</title>
...
<!-- 			<button class="tablinks" onclick="openTab(event, 'tab2')">구매완료</button> -->
			<button class="tablinks" onclick="location.href='ItemOrdMng.shop'">구매진행</button>
			<button class="tablinks active" onclick="location.href='ItemOrdFinishMng.shop'">구매완료</button>
		</div>
				...
				<div class ="paging">
					<c:choose>
						<c:when test="${pageInfo.pageNum > 1 }">
							<input type="button" value=" < " onclick="location.href='ItemOrdFinishMng.shop?pageNum=${pageInfo.pageNum - 1 }'">
						</c:when>
						<c:otherwise>
					...
					<c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
						...
						<c:otherwise>
							<a href="ItemOrdFinishMng.shop?pageNum=${i }">${i }</a>
						</c:otherwise>
					</c:choose>
					</c:forEach>
					<c:choose>
